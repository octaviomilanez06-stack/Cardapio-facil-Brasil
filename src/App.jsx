...
const DEFAULT_STORE = {
  name: "Cardápio Fácil Brasil", category: "Delivery",
  delivery_fee: 5.00, min_order: 25.00,
  delivery_time: "30–50 min", prep_time: "15–25 min",
  banner: "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=1200&q=80",
  logo: "🍔", is_open: true,
};

const INITIAL_PRODUCTS = [
  { name: "Classic Smash Burger", category: "Hambúrgueres", price: 32.9, description: "Dois smash patties 90g, queijo cheddar, pickles, cebola roxa e molho especial.", image: "https://images.unsplash.com/photo-1550317138-10000687a72b?w=400&q=80", tag: "bestseller", active: true },
  { name: "Double Bacon Crispy", category: "Hambúrgueres", price: 42.9, description: "Dupla blend especial, bacon crocante, queijo coalho, alface, tomate e maionese defumada.", image: "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?w=400&q=80", tag: "new", active: true },
  { name: "Veggie Deluxe", category: "Hambúrgueres", price: 29.9, description: "Hambúrguer de grão-de-bico, queijo brie, rúcula, tomate seco e pesto.", image: "https://images.unsplash.com/photo-1520072959219-c595dc870360?w=400&q=80", tag: null, active: true },
  { name: "Batata Frita Rústica", category: "Acompanhamentos", price: 18.9, description: "Batatas rústicas temperadas com alecrim e flor de sal, com molho aioli.", image: "https://images.unsplash.com/photo-1529990098630-4022df7bb7cc?w=400&q=80", tag: "promo", active: true },
  { name: "Onion Rings Premium", category: "Acompanhamentos", price: 22.9, description: "Anéis de cebola empanados em panko crocante com molho ranch.", image: "https://images.unsplash.com/photo-1639024471283-03518883512d?w=400&q=80", tag: null, active: true },
  { name: "Milkshake Oreo", category: "Bebidas", price: 24.9, description: "Milkshake cremoso com sorvete de baunilha, biscoito Oreo e chantilly.", image: "https://images.unsplash.com/photo-1572490122747-3968b75cc699?w=400&q=80", tag: "bestseller", active: true },
  { name: "Coca-Cola 600ml", category: "Bebidas", price: 9.9, description: "Coca-Cola gelada.", image: "https://images.unsplash.com/photo-1629203851122-3726555cf519?w=400&q=80", tag: null, active: true },
  { name: "Brownie com Sorvete", category: "Sobremesas", price: 19.9, description: "Brownie quentinho de chocolate belga com sorvete de baunilha e calda de caramelo.", image: "https://images.unsplash.com/photo-1564355808539-22fda35bed7e?w=400&q=80", tag: "new", active: true },
  { name: "Combo Família", category: "Combos", price: 89.9, description: "2 Classic Smash + 1 Double Bacon + 2 Batatas Grandes + 4 Refris 350ml.", image: "https://images.unsplash.com/photo-1561758033-48d52648ae8b?w=400&q=80", tag: "promo", active: true },
];
...
function ImageUpload({ value, onChange, style }) {
  ...
  return (
    <div style={style}>
      ...
      <div onClick={() => inputRef.current.click()} style={{ border: "2px dashed #F0F0EE", borderRadius: 12, padding: 14, textAlign: "center", cursor: "pointer", background: "#F7F7F5" }} onMouseEnter={e => e.currentTarget.style.borderColor = "#FF4D00"} onMouseLeave={e => e.currentTarget.style.borderColor = "#F0F0EE"}>
...
function Spinner() {
  return <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100vh" }}><div style={{ width: 40, height: 40, border: "4px solid #F0F0EE", borderTop: "4px solid #FF4D00", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} /><style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style></div>;
}
      ...
      <div style={{ maxWidth: 500, margin: "0 auto", padding: 24 }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {["delivery", "retirada"].map(t => (
              <button key={t} onClick={() => setOrderType(t)} style={{ flex: 1, padding: 12, borderRadius: 12, border: `2px solid ${orderType === t ? "#FF4D00" : "#F0F0EE"}`, background: orderType === t ? "#FFF5F0" : "#fff", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 16 }}>
          ...
          {orderType === "delivery" && (
            <div>
              ...
              {customerInfo.address && <a href={`https://maps.google.com/?q=${encodeURIComponent(customerInfo.address)}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#FF4D00", fontWeight: 600 }}>🗺️ Ver no Google Maps</a>}
            </div>
          )}
        </div>
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, padding: 20, marginBottom: 20 }}>
          ...
          <div style={{ borderTop: "1px solid #F0F0EE", marginTop: 12, paddingTop: 12 }}>
            {orderType === "delivery" && <div style={{ display: "flex", justifyContent: "space-between", fontSize: 14, marginBottom: 6, color: "#6B7280" }}><span>Taxa de entrega</span><span>R$ {Number(store.delivery_fee).toFixed(2)}</span></div>}
    ...
    <div style={{ minHeight: "100vh", background: "#F7F7F5" }}>
      ...
      <div style={{ position: "relative", height: 240 }}>
        ...
        <div style={{ position: "absolute", bottom: 20, left: 20, right: 20, color: "#fff" }}>
          ...
          <div style={{ display: "flex", gap: 14, fontSize: 12, flexWrap: "wrap" }}>
            ...
            <span>⏱ Preparo: {store.prep_time}</span>
            <span>📦 Mín. R$ {Number(store.min_order).toFixed(2)}</span>
            <span>🛵 R$ {Number(store.delivery_fee).toFixed(2)}</span>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #F0F0EE", overflowX: "auto", whiteSpace: "nowrap", padding: "0 16px", position: "sticky", top: 0, zIndex: 100 }}>
        <div style={{ display: "inline-flex", gap: 4, padding: "10px 0" }}>
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} style={{ padding: "8px 16px", borderRadius: 24, border: "none", cursor: "pointer", fontFamily: "'Syne', sans-serif", fontWeight: 600, fontSize: 13, background: activeCategory === cat ? "#FF4D00" : "#F0F0EE", color: activeCategory === cat ? "#fff" : "#1A1A1A", whiteSpace: "nowrap" }}>{cat}</button>
          ))}
        </div>
      </div>
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px 16px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: 16 }}>
          {filtered.map(p => (
            <div key={p.id} onClick={() => setSelectedProduct(p)} style={{ background: "#fff", borderRadius: 18, overflow: "hidden", cursor: "pointer", boxShadow: "0 2px 12px rgba(0,0,0,0.07)", transition: "transform 0.2s" }} onMouseEnter={e => e.currentTarget.style.transform = "translateY(-3px)"} onMouseLeave={e => e.currentTarget.style.transform = "translateY(0)"}>
              ...
              <div style={{ padding: 16 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{ fontSize: 17, fontWeight: 800, color: "#FF4D00", fontFamily: "'Syne', sans-serif" }}>R$ {Number(p.price).toFixed(2)}</span>
                  <button onClick={e => { e.stopPropagation(); addToCart(p); }} style={{ background: "#FF4D00", color: "#fff", border: "none", borderRadius: 10, width: 36, height: 36, fontSize: 20, cursor: "pointer" }}>+</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setSelectedProduct(null)}>
          <div style={{ background: "#fff", borderRadius: "22px 22px 0 0", width: "100%", maxWidth: 560, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ position: "relative", height: 260 }}>
              ...
              <button onClick={() => setSelectedProduct(null)} style={{ position: "absolute", top: 14, right: 14, background: "rgba(0,0,0,0.5)", color: "#fff", border: "none", borderRadius: "50%", width: 34, height: 34, cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ padding: 24 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <span style={{ fontSize: 22, fontWeight: 800, color: "#FF4D00", fontFamily: "'Syne', sans-serif" }}>R$ {Number(selectedProduct.price).toFixed(2)}</span>
                <button onClick={() => { addToCart(selectedProduct); setSelectedProduct(null); }} style={{ background: "#FF4D00", color: "#fff", border: "none", borderRadius: 12, padding: "13px 26px", fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 14, cursor: "pointer" }}>Adicionar</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 500, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => setShowCart(false)}>
          <div style={{ background: "#fff", borderRadius: "22px 22px 0 0", width: "100%", maxWidth: 560, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #F0F0EE", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setShowCart(false)} style={{ background: "#F0F0EE", border: "none", borderRadius: "50%", width: 34, height: 34, cursor: "pointer" }}>✕</button>
            </div>
...
function AdminArea({ products, setProducts, store, setStore }) {
  ...
  async function saveNewProduct() {
    ...
    setSaving(true);
    const result = await db("products", "POST", { ...newProduct, price: parseFloat(newProduct.price), active: true, image: newProduct.image || "https://images.unsplash.com/photo-1550317138-10000687a72b?w=400&q=80" });
  ...
  async function saveStore() {
    ...
    setSaving(false);
    notify("Loja salva com sucesso!");
  }

  async function toggleStore() {
    const newVal = !store.is_open;
    await db("store", "PATCH", { is_open: newVal }, "?id=eq.1");
    setStore(prev => ({ ...prev, is_open: newVal }));
    notify(newVal ? "Loja aberta!" : "Loja fechada!");
  }
  ...
  return (
    <div style={{ display: "flex", height: "100vh", background: "#F7F7F5", overflow: "hidden" }}>
      ...
      <div style={{ width: sidebarOpen ? 220 : 60, background: "#1A1A1A", display: "flex", flexDirection: "column", transition: "width 0.3s", overflow: "hidden", flexShrink: 0 }}>
        <div style={{ padding: "18px 14px", borderBottom: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          {sidebarOpen && <span style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 14, color: "#fff", whiteSpace: "nowrap" }}>Admin</span>}
        ...
        <nav style={{ flex: 1, padding: "10px 6px" }}>
          {MENU.map(item => (
            <button key={item.id} onClick={() => setSection(item.id)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "11px 10px", borderRadius: 10, border: "none", cursor: "pointer", background: section === item.id ? "#FF4D00" : "transparent", color: section === item.id ? "#fff" : "rgba(255,255,255,0.6)", fontWeight: section === item.id ? 700 : 400, marginBottom: 3, textAlign: "left" }}>
        ...
        <div style={{ padding: 24 }}>
          ...
          {section === "products" && (
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 20 }}>
                ...
                <button onClick={() => setShowAddProduct(true)} style={{ background: "#FF4D00", color: "#fff", border: "none", borderRadius: 12, padding: "10px 20px", fontWeight: 700, cursor: "pointer" }}>+ Novo Produto</button>
              </div>
              ...
              {showAddProduct && (
                <div style={{ background: "#fff", borderRadius: 16, padding: 24, marginBottom: 20, boxShadow: "0 4px 24px rgba(0,0,0,0.1)" }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                    <div><label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Nome *</label><input value={newProduct.name} onChange={e => setNewProduct(p => ({ ...p, name: e.target.value }))} style={inputStyle} /></div>
                    <div><label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Preço *</label><input value={newProduct.price} onChange={e => setNewProduct(p => ({ ...p, price: e.target.value }))} style={inputStyle} /></div>
                  </div>
                  <label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Categoria</label>
                  <select value={newProduct.category} onChange={e => setNewProduct(p => ({ ...p, category: e.target.value }))} style={inputStyle}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select>
                  ...
                  <textarea value={newProduct.description} onChange={e => setNewProduct(p => ({ ...p, description: e.target.value }))} style={{ ...inputStyle, height: 70, resize: "none" }} />
                  <label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Foto</label>
                  <ImageUpload value={newProduct.image} onChange={img => setNewProduct(p => ({ ...p, image: img }))} style={{ marginBottom: 16 }} />
                  <div style={{ display: "flex", gap: 10 }}>
                    <button onClick={saveNewProduct} disabled={saving} style={{ background: "#FF4D00", color: "#fff", border: "none", borderRadius: 10, padding: "10px 20px", fontWeight: 700, cursor: "pointer" }}>{saving ? "Salvando..." : "Salvar"}</button>
              ...
              {editingProduct && (
                <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }} onClick={() => setEditingProduct(null)}>
                  <div style={{ background: "#fff", borderRadius: 20, padding: 28, width: "100%", maxWidth: 500, maxHeight: "90vh", overflow: "auto" }} onClick={e => e.stopPropagation()}>
                    <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, marginBottom: 20 }}>Editar Produto</h3>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                      <div><label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Nome *</label><input value={editingProduct.name} onChange={e => setEditingProduct(p => ({ ...p, name: e.target.value }))} style={inputStyle} /></div>
                      <div><label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Preço *</label><input value={editingProduct.price} onChange={e => setEditingProduct(p => ({ ...p, price: e.target.value }))} style={inputStyle} /></div>
                    </div>
                    <label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Categoria</label>
                    <select value={editingProduct.category} onChange={e => setEditingProduct(p => ({ ...p, category: e.target.value }))} style={inputStyle}>{CATEGORIES.map(c => <option key={c}>{c}</option>)}</select>
                    <label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 4 }}>Tag</label>
                    ...
                    <div style={{ display: "flex", gap: 10 }}>
                      <button onClick={saveEditProduct} disabled={saving} style={{ background: "#FF4D00", color: "#fff", border: "none", borderRadius: 10, padding: "12px 24px", fontWeight: 700, cursor: "pointer", flex: 1 }}>{saving ? "Salvando..." : "Salvar Alterações"}</button>
                  ...
                  <div key={p.id} style={{ background: "#fff", borderRadius: 16, overflow: "hidden", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", opacity: p.active ? 1 : 0.55 }}>
                    ...
                    <div style={{ padding: 14 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => setEditingProduct({ ...p })} style={{ flex: 1, background: "#EFF6FF", border: "none", borderRadius: 8, padding: 7, fontSize: 12, fontWeight: 700, cursor: "pointer", color: "#1D4ED8" }}>✏️ Editar</button>
                        ...
                        <button onClick={() => deleteProduct(p.id)} style={{ background: "#F0F0EE", border: "none", borderRadius: 8, padding: "7px 10px", cursor: "pointer" }}>🗑️</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {section === "store" && (
            <div style={{ maxWidth: 600 }}>
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 20 }}>
                ...
                {[["name", "Nome da Loja"], ["category", "Categoria"], ["delivery_time", "Tempo de Entrega"], ["prep_time", "Tempo de Preparo"]].map(([field, label]) => (
                  <div key={field} style={{ marginBottom: 14 }}>
                    <label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 6 }}>{label}</label>
                    <input value={store[field] || ""} onChange={e => setStore(p => ({ ...p, [field]: e.target.value }))} style={inputStyle} />
                  </div>
                ))}
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div><label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 6 }}>Taxa de Entrega (R$)</label><input type="number" step="0.01" value={store.delivery_fee} onChange={e => setStore(p => ({ ...p, delivery_fee: e.target.value }))} style={inputStyle} /></div>
                  <div><label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 6 }}>Pedido Mínimo (R$)</label><input type="number" step="0.01" value={store.min_order} onChange={e => setStore(p => ({ ...p, min_order: e.target.value }))} style={inputStyle} /></div>
                </div>
              </div>

              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 20 }}>
                <h3 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, marginBottom: 16 }}>🖼️ Banner da Loja</h3>
                <ImageUpload value={store.banner?.startsWith("data:") ? store.banner : null} onChange={img => setStore(p => ({ ...p, banner: img }))} style={{ marginBottom: 14 }} />
                <label style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", display: "block", marginBottom: 6 }}>Ou cole um link de imagem</label>
                <input placeholder="https://..." value={store.banner?.startsWith("data:") ? "" : (store.banner || "")} onChange={e => setStore(p => ({ ...p, banner: e.target.value }))} style={inputStyle} />
              </div>
              ...
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)", marginBottom: 20 }}>
                ...
                <input placeholder="🍔" value={(store.logo?.startsWith("data:") || store.logo?.startsWith("http")) ? "" : (store.logo || "")} onChange={e => setStore(p => ({ ...p, logo: e.target.value }))} style={{ ...inputStyle, fontSize: 24, textAlign: "center" }} />
              ...
              <button onClick={saveStore} disabled={saving} style={{ width: "100%", background: "#FF4D00", color: "#fff", border: "none", borderRadius: 14, padding: 16, fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: 16, cursor: "pointer", marginBottom: 16 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 16, padding: 24, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
                ...
                <button onClick={toggleStore} style={{ background: store.is_open ? "#FEE2E2" : "#D1FAE5", border: "none", borderRadius: 12, padding: "14px 28px", fontWeight: 800, cursor: "pointer", fontSize: 15, color: store.is_open ? "#991B1B" : "#065F46", fontFamily: "'Syne', sans-serif" }}>
          ...
          {section === "orders" && (
            <div style={{ textAlign: "center", padding: 60, color: "#9B9B9B" }}>
              ...
              <p style={{ fontFamily: "'Syne', sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 12 }}>Pedidos via WhatsApp</p>
              <p style={{ fontSize: 14, maxWidth: 400, margin: "0 auto", lineHeight: 1.7 }}>Quando um cliente finaliza o pedido, você recebe uma mensagem no WhatsApp <strong style={{ color: "#FF4D00" }}>(21) 97701-6114</strong> com todos os detalhes: nome, endereço, link do Maps, itens, total e pagamento.</p>
            </div>
          )}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1A1A1A", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 20, padding: 40, width: "100%", maxWidth: 380, textAlign: "center", boxShadow: "0 20px 60px rgba(0,0,0,0.3)" }}>
        ...
        <input type="password" placeholder="Digite a senha" value={pwd} onChange={e => setPwd(e.target.value)} onKeyDown={e => e.key === "Enter" && tryLogin()} style={{ width: "100%", border: `2px solid ${error ? "#EF4444" : "#F0F0EE"}`, borderRadius: 12, padding: "13px 16px", fontSize: 15, outline: "none", marginBottom: 14, textAlign: "center", letterSpacing: 4 }} />
